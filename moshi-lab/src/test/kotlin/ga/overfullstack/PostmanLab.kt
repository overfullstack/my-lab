...
class PostmanLab {
  ...
  fun `parse postman environment`() {
    ...
    env!!.values.asSequence()
      .filter { (key) ->
        !whiteListToRetain.contains(key) && !key.endsWith("PsgId") && !key.endsWith("UserId") && key.endsWith(
          "Id"
        )
      }
  ...
  fun `override env from file`() {
    val userKeys =
      setOf("taxAdminUserName", "billingAdminUserName", "productAndPricingAdminUserName", "salesRepUserName")
    val anyAdapter = Moshi.Builder().build().adapter<Any>()
    val env =
      anyAdapter.fromJson(readFileFromTestResource("postman/pq-user-env.postman_environment.json")) as Map<String, Any>
    val values = env.get("values") as List<Map<String, String>>
    val envMap = values.asSequence().map { it["key"] to it["value"] }.toMap()
    val epoch = System.currentTimeMillis()
    val overriddenMap = envMap.asSequence().map {
      if (userKeys.contains(it.key)) 
        it.key to it.value?.let { targetStr ->  targetStr.insertStringAtIndex("-$epoch", targetStr.indexOf("@") - 1) }

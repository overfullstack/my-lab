...
class MilestoneLab {
	...
	void milestone() {
		...
		final var cNBDEqualsCancellationDate =
				"("
						+ nonNullNBD
						+ " AND "
						+ nonNullCancellationDate
						+ " AND "
						+ nbdEqualsCancellationDate
						+ ")";
		final var cancelledBSWithDateBasedBMPIBeyondCancellation =
				"("
						+ cNBDEqualsCancellationDate
						+ " AND "
						+ dateBasedBMPI
						+ " AND "
						+ nonNullAccomplishmentDate
						+ " AND "
						+ bmpisBeyondCancellationDate
						+ ")";
		final var cancelledBSWithUnaccomplishedEventBasedBMPI =
				"("
						+ cNBDEqualsCancellationDate
						+ " AND "
						+ eventBasedBMPI
		...
		final var filterCriteriaArrangement =
				"("
						...
						+ " AND ("
						+ "("
						+ cancelledBSWithDateBasedBMPIBeyondCancellation
						+ " OR "
						+ cancelledBSWithUnaccomplishedEventBasedBMPI
						+ ") OR ("
						+ readyForInvoicingBMPI
						+ ")"
						+ "))"
						+ " OR "
						+ amendmentBS
						...
						+ cancellationBS
						+ ")))";
		// (14 OR (1 AND ((2 AND ((((11 AND 5 AND 12) AND 7 AND 6 AND 13) OR ((11 AND 5 AND 12) AND 8
		// AND 9)) OR (10))) OR 3 OR 4)))
		assertEquals(
				"(14 OR (1 AND ((2 AND ((((11 AND 5 AND 12) AND 7 AND 6 AND 13) OR ((11 AND 5 AND 12) AND 8 AND 9)) OR (10))) OR 3 OR 4)))",
				filterCriteriaArrangement);
	}
}
